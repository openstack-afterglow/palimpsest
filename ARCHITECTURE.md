...
Build cap은 같은 store의 project별 filesystem flock 안에서 SQL count+insert를 commit해 동시 POST가 4개를 넘지 못하게 한다. 같은 digest를 두 project가 동시에 등록할 때는 digest별 flock으로 descriptor 조회·grant/insert·commit을 직렬화한다. Bundle member bytes는 ingest 전에 선언 digest와 비교하며 기존 공유 blob을 삭제하지 않는다. Range `bytes=-N`은 마지막 N바이트를 뜻하고 `-0`은416이다.
...
API는 401 Keystone validation, 403 system-admin, 404 visibility/ownership, 409 offset/descriptor conflict, 413 size limit, 422 digest/schema 오류를 구분한다. Hub build가 설정되지 않았으면 POST는 503, project별 build queue/활성 upload cap을 넘기면 429다. Upload는 file lock에서 last-acknowledged offset으로 residue를 reconcile하고 fsync 뒤 DB offset을 commit한다. Local runtime은 foreign domain, stale/ambiguous ledger, failed ACL/release를 성공으로 제조하지 않는다. 실패한 OCI materializer가 즉시 reap되지 않으면 scratch authority를 background reaper가 보존하므로 임의 삭제하지 않는다. stage-1 partial root transition은 rollback 성공으로 표시하지 않으며, exact evidence가 없으면 fail-closed한다.

Upload lock 대기 중 요청이 취소되면 background flock의 eventual FD를 반환하며, blob 승격 뒤에도 staged upload bytes를 SQL registration commit까지 별개 파일로 보존한다. Commit 전 실패는 동일 offset/metadata로 PUT을 다시 시도할 수 있다. 성공한 commit 뒤 staging을 제거한다. 이 보존은 중간에 임시 bytes와 blob bytes가 동시에 차지하는 저장공간을 요구한다.

`PALIMPSEST_HUB_MAX_BLOB_BYTES`는 blob 한 개의 상한이며 project/store 총량 quota는 없다. 완료된 build row/result blob의 자동 만료·GC도 없으므로 외부 filesystem 용량·사용량 관리가 운영 전제다.
