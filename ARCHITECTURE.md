...
정확한 Linux checkout `fac2ec594f7f03e1ec5745babbf8337ebc7568c3`에서 service contract 30건(1.85초)과 PyTorch loopback HTTP inference service native case 1건(279.07초)이 통과했고 GitHub development package workflow `34944305626`도 통과했다. 공개 detached run `ml-pytorch-service-b336089f`는 guest `127.0.0.1:18080`에서 deterministic six-layer/256-wide Transformer를 제공했고, 공개 `exec --timeout 300`이 health 및 동일 inference 2건의 HTTP 200·증가 counter·finite `[1,128,256]` CPU output·CUDA false·동일 SHA-256 `73cf2a3cfaf2e95a0962b15c4eb8d259760ad5bf3a370562ec2c9296a38dc464`를 확인했다. CPU-only XML, 인증 root device21/inode2, PID1 거부, source hash 불변, proof-owned stop/rm과 run/root-volume 제거도 통과했다. 새 service domain은 postflight에 없고 기존 22-domain inventory와 archive16개 hash는 보존됐으며, 이전 실패의 running `ml-pytorch-ed03b448`은 건드리지 않았다. 이는 guest-loopback 실제 서비스 실행만 live-verify한다. `network none`이므로 host/외부 reachability, pretrained model 품질, CUDA/GPU helper·attach·rebind는 여전히 미검증이다. 보존 evidence `/tmp/pms-a/m-434a58e7` 때문에 `/tmp` 여유가 36,758,560,768 bytes로 40GiB next-run precondition 아래이므로 다음 대용량 proof 전에 별도 storage 판단이 필요하다.
...
{
  "schema_version": 1,
  "source_sha256": "8166ea35f2552e86460c70fe17420fb378c4b42dde1960b52dd0bdfa8857c0c8",
  "reviewed_at": "2026-09-15T10:14:28Z",
  "summary": "Reviewed selectable OCI-root guest networking: QEMU user-mode NAT/host-only/none with explicit IPv4 publications bound into domain core v10, domain plan v16, and stage-1 plan v16, PID 1 NIC/route/resolver verification, read-only exposure projection, and explicit limits on libvirt resources, IPv6, privileged ports, and VM-to-VM networking."
}
```
