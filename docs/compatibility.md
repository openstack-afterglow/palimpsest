...
| Host Platform: macOS arm64 (Experimental) | Supported (`libvirt-hvf`) | Explicit `--backend libvirt-hvf` for conventional aarch64 cloud images only: Hypervisor.framework via `qemu:///session`, `virt`/GICv3/UEFI, `hdiutil` seed ISO and SLIRP loopback SSH `hostfwd` on a virtio-mmio NIC (no libvirt network driver or competing PCI slot). Owned EFI NVRAM is removed with the owned run tree. The default remains Lima/VZ; OCI-root is Linux KVM only. |
