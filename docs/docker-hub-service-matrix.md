...
The separately approved `MYSQL_USER_RANDOM_PASSWORD` test derives a temporary
OCI archive from its independently pinned original MySQL archive. It changes
only the config entrypoint to fixed guest-side Bash that reads 32 bytes from
`/dev/urandom`, validates a 64-character lowercase hexadecimal value, exports a
`palimpsest-test-`-prefixed password inside the workload, and execs the original
entrypoint and `mysqld` command. Generated bytes are never host input or archive,
argv, or ledger content. Command output is checked before retained persistence;
the runtime console is bounded, leak-detected, and retained only as a redacted
copy before disposal. The test requires initialization completion
followed by a later final-server readiness marker; its unauthenticated Unix-socket
`mysqladmin ping` is reachability only, not authenticated SQL. It always attempts
public stop/removal of only its bound new run and verifies the root-volume
...
The random-password diagnostic is the explicit exception to failed-runtime
retention because its whole newly owned root is disposable by user approval;
historical failed runs remain untouched.
