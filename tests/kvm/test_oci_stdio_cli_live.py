...
def test_public_stdio_fd_ownership_and_reopen_diagnostic(uid: int) -> None:
    ...
    assert all(main[f"fd{fd}type"] == 0o010000 for fd in (1, 2))
    assert all(main[f"fd{fd}uid"] == uid for fd in (1, 2))
    assert all(main[f"fd{fd}gid"] == uid for fd in (1, 2))
    assert all(main[f"fd{fd}mode"] == 0o600 for fd in (1, 2))
    assert all(main[f"fd{fd}reopen"] == 0 for fd in (1, 2))
