...
def harness(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    generated = template.replace("/* READ_CONTROL_FUNCTION */", read_frame).replace(
        "/* LIFECYCLE_PUMP_FUNCTION */", pump
    )
...
@pytest.mark.parametrize("scenario", ["eintr-empty", "eintr-partial", "pump-yields-64"])
