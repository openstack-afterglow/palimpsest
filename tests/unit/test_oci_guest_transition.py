...
static __attribute__((noreturn, used)) void harness_main(u64 *stack) {
    ...
    } else if (text_equal(kind, "regular")) {
    ...
    if (text_equal(kind, "generic-mode")) {
        if (safe_dir_checked(target, 1, 1, 0755, &fd, &reason)) exit_now(99);
        if (reason != SAFE_DIR_REASON_MODE) exit_now(95);
        exit_now(0);
    }
    if (transition_target_policy_checked(target, label, 1, &fd, &initial_identity, &reason)) {
        if (text_equal(kind, "ready")) {
            ...
            if (sc2(SYS_chmod, (i64)target, 0555) != 0 ||
                transition_target_ready_checked(target, label, fd, &initial_identity, 0x01021994)) exit_now(97);
        } else if (text_equal(kind, "ready-to-0755")) {
            if (sc2(SYS_chmod, (i64)target, 0755) != 0 ||
                transition_target_ready_checked(target, label, fd, &initial_identity, 0x01021994)) exit_now(98);
        } else if (text_equal(kind, "ready-wrong-fs")) {
            if (transition_target_ready_checked(target, label, fd, &initial_identity, 0x794c7630)) exit_now(96);
        } else if (text_equal(kind, "ready-replaced")) {
    ...
    if (text_equal(kind, "owner-nonempty") && reason == SAFE_DIR_REASON_OWNER) exit_now(0);
    if (text_equal(kind, "regular") && reason == SAFE_DIR_REASON_OPEN) exit_now(0);
    exit_now(95);
}
...
def test_real_c_transition_target_accepts_only_approved_modes(transition_harness, target, mode):
    result = transition_harness("accepted", target=target, fixture_mode=mode)
    assert result.returncode == 0, result.stderr.decode(errors="replace")
    assert result.stdout == b""
