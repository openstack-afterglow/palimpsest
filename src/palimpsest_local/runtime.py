...
def _get_domain_run_id(domain: Any) -> str | None:
    """Extract palimpsest run_id metadata from a libvirt domain object."""
    try:
        libvirt = kvm._libvirt()
        meta_xml = domain.metadata(
            libvirt.VIR_DOMAIN_METADATA_ELEMENT,
            kvm.DOMAIN_MARKER_NAMESPACE,
        )
        if meta_xml:
    ...
    try:
        ...
        elem = root.find(f"./metadata/{{{kvm.DOMAIN_MARKER_NAMESPACE}}}run")
        if elem is not None:
            return elem.get("id")
    except Exception:
        pass
            ...
            if conn_obj is not None:
                try:
                    ...
                    if libvirt_domain is not None:
                        domain_run_id = _get_domain_run_id(libvirt_domain)
        ...
        try:
            ...
            if conn_obj is not None:
                try:
                    ...
                    if domain is not None and _get_domain_run_id(domain) == run_id:
