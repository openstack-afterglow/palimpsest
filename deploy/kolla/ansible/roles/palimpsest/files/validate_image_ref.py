#!/usr/bin/env python3
"""Validate Palimpsest OCI image references before deployment mutation."""
...
def validate_image_ref(reference: str, *, source_mode: bool) -> None:
    if source_mode:
        if not _LOCAL_SOURCE_REF.fullmatch(reference):
            raise ValueError(
                "invalid Palimpsest image reference; "
                "expected afterglow-local/palimpsest-hub-{api|worker}:<12-hex-commit>"
            )
        return
    ...
    if tag_match:
        ...
        if tag.lower() == "latest":
            raise ValueError(
                "invalid Palimpsest image reference; latest tag is prohibited, "
